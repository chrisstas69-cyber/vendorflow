...
export class OutdoorMoviesLIScraper extends BaseScraper {
  constructor() {
    super('outdoor-movies-li', 'https://mommypoppins.com/long-island-kids/outdoor-movies', 'Long Island');
  }
  ...
  parse(html: string): ScrapedEvent[] {
    ...
    $('article, .event-item, .entry, .post, [class*="event"], .node, .views-row').each((_, el) => {
      try {
        const $el = $(el);
