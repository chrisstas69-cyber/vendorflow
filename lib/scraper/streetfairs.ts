...
export class StreetFairsScraper extends BaseScraper {
  constructor() {
    super('streetfairs-org', 'https://streetfairs.org', 'NJ');
  }
  ...
  parse(html: string): ScrapedEvent[] {
    ...
    $('table tr, .event-item, .fair-listing, article, .entry, .post').each((_, el) => {
      try {
        const $el = $(el);
