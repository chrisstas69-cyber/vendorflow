...
export class NJParentingScraper extends BaseScraper {
  constructor() {
    super('njparenting', 'https://njparenting.com/events/', 'NJ');
  }
  ...
  parse(html: string): ScrapedEvent[] {
    ...
    $('article, .event-item, .entry, .post, [class*="event"]').each((_, el) => {
      try {
        const $el = $(el);
