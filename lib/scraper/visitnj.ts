...
export class VisitNJScraper extends BaseScraper {
  constructor() {
    super('visitnj', 'https://visitnj.org/events', 'NJ');
  }
  ...
  parse(html: string): ScrapedEvent[] {
    ...
    $('article, .event-item, .views-row, .event-card, .node--type-event').each((_, el) => {
      try {
        const $el = $(el);
        ...
        const location = $el.find('.location, .venue, .field--name-field-location').first().text().trim();
