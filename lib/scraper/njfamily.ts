...
export class NJFamilyScraper extends BaseScraper {
  constructor() {
    super('njfamily', 'https://njfamily.com/events/', 'NJ');
  }
  ...
  parse(html: string): ScrapedEvent[] {
    ...
    $('article, .event-item, .entry, .post, .tribe-events-calendar-list__event').each((_, el) => {
      try {
        const $el = $(el);
        const title = $el.find('h2, h3, .tribe-events-calendar-list__event-title, a').first().text().trim();
