    ...
    <div className={`min-h-screen flex ${shell}`}>
      <aside className={`hidden lg:flex w-56 flex-col border-r shrink-0 ${sidebar}`}>
        ...
        <div className={`p-3 border-t space-y-2 ${dark ? 'border-gray-800' : 'border-gray-200'}`}>
          <PublicThemeToggle />
          <RoleSwitcher accent="amber" />
        </div>
      </aside>

      <div className="flex-1 flex flex-col min-w-0">
        <header
          className={`lg:hidden sticky top-0 z-40 border-b px-4 py-3 flex items-center justify-between ${sidebar}`}
        >
          <Link href="/pulse" className="font-bold text-sm">
            Vendor Hub
          </Link>
