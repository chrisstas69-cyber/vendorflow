...
export default function WeekEventsPage() {
  const [events, setEvents] = useState<Event[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/events/list?view=week')
      .then(r => r.json())
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <h1 className="text-2xl font-bold mt-2 mb-6">This Week</h1>

        {loading ? (
          <div className="text-gray-500">Loading...</div>
        ) : events.length === 0 ? (
          <div className="text-gray-500 text-center py-12">No events this week.</div>
        ) : (
              ...
              return (
                <div key={e.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-xs text-blue-400 font-medium">{dayName}, {e.event_date}</div>
